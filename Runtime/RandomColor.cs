...
namespace RandomColorUnity
{
    public class RandomColor
    {
        ...
        static readonly Dictionary<PredefinedColor, ColorBounds> s_ColorPalette = new Dictionary<PredefinedColor, ColorBounds> {
            { PredefinedColor.Red, new ColorBounds(new Range(-26, 18), new Range(20, 100), new[] { 100, 92, 89, 85, 78, 70, 60, 55, 50 }) },
            { PredefinedColor.Orange, new ColorBounds(new Range(19, 46), new Range(20, 100), new[] { 100, 93, 88, 86, 85, 70, 70 }) },
            { PredefinedColor.Yellow, new ColorBounds(new Range(47, 62), new Range(25, 100), new[] { 100, 94, 89, 86, 84, 82, 80, 75 }) },
            { PredefinedColor.Green, new ColorBounds(new Range(63, 178), new Range(30, 100), new[] { 100, 90, 85, 81, 74, 64, 50, 40 }) },
            { PredefinedColor.Blue, new ColorBounds(new Range(179, 257), new Range(20, 100), new[] { 100, 86, 80, 74, 60, 52, 44, 39, 35 }) },
            { PredefinedColor.Purple, new ColorBounds(new Range(258, 282), new Range(20, 100), new[] { 100, 87, 79, 70, 65, 59, 52, 45, 42 }) },
            { PredefinedColor.Pink, new ColorBounds(new Range(283, 334), new Range(20, 100), new[] { 100, 90, 86, 84, 80, 75, 73}) }
        };

        static readonly ColorBounds s_Monochrome = new ColorBounds(null, null, new[] { 0, 0 });
        ...
        int RandomWithin(Range range)
        {
            return m_Generator.Next(range.Min, range.Max);
        }
